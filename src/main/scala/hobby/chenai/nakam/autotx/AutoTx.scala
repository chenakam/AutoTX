...
import hobby.chenai.nakam.autotx.core.coin.CnyGroup.wrapCnyNum
...
object AutoTx {
  ...
  def entryOrdersIntelligent(): Unit = {
    ...
    if (是) {
      ...
      val 买 = true
      val 目标 = 具备最佳时机的(优质币种)
      if (目标.nonEmpty) autoEntryOrdersOfBuying(买, 额度, 目标)
      else {
  ...
  // 具体的一个订单
  def entryOrderOfBuying(buyOrSell: Boolean, money: AbsCashGroup#COIN, token: AbsTokenGroup): Unit = {
  ...
  def autoEntryOrdersOfBuying(买or卖: Boolean, 金额: AbsCashGroup#COIN, 优质币种: Seq[(AbsTokenGroup, Double)]): Unit = {
    优质币种.foreach { 币 =>
      // TODO: 拆分成几单，需要计算最优解。
      val each = 金额 / 10
      (0 until 10).foreach { _ => entryOrderOfBuying(买or卖, each * 币._2, 币._1) }
    }
  }
  ...
  def 是否继续投入资金: (Boolean, AbsCashGroup#COIN) = (可用现金额 >= 可挂单最低现金额, 可用现金额)

  def 可挂单最低现金额: AbsCashGroup#COIN = 50 CNY

  def 可用现金额: AbsCashGroup#COIN = 现金总余额 - 现金备用预留

  def 现金总余额: AbsCashGroup#COIN = 10e6 CNY

  def 现金备用预留: AbsCashGroup#COIN = 10e5 CNY
