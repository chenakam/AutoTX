...
/**
  ...
  */
object SrmToken extends AbsTokenGroup {
  override type COIN = Serum
  override type UNIT = COIN with Unt

  override def unitStd = SRM

  override def make(count: BigInt, unt: UNIT) = new Serum(count) {
    override def unit = unt
  }

  abstract class Serum private[SrmToken](count: BigInt) extends AbsCoin(count: BigInt) {
    override def equals(obj: Any) = obj match {
      case that: Serum => that.canEqual(this) && that.count == this.count
      case _ => false
    }

    override def canEqual(that: Any) = that.isInstanceOf[Serum]
  }

  lazy val SRM: UNIT = new Serum(10.pow(8)) with Unt {
